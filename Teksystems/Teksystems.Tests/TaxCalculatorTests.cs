...
namespace Teksystems.Tests
{
    public class TaxCalculatorTests
    {
        ...
        public async Task CalculateTax_WhenOnlySalesTax_IncludesSalesTax(decimal salesTax, decimal itemPrice, decimal expectedTaxes)
        {
            var item = new Item("Test", itemPrice);
            var calculator = new TaxCalculator(salesTax, 1);

            var result = await calculator.CalculateTaxAsync(item, 1);
        ...
        [InlineData(0.12, 12.34, 13.84)]
        [InlineData(0, 10.0, 10.0)]
        public async Task CalculateTax_WhenOnlyImportTax_IncludesImportTax(decimal importTax, decimal itemPrice, decimal expectedTaxes)
        {
        ...
        public async Task CalculateTax_WhenBothTax_IncludesBothTax(decimal salesTax, decimal importTax, decimal itemPrice, decimal expectedTaxes)
        {
        ...
        public async Task CalculateTax_WhenItemNotTaxed_ReturnsZero(decimal salesTax, decimal importTax, decimal itemPrice)
        {
        ...
        public async Task CalculateTax_WhenMoreThanOneItem_CalculatesTaxOnTotal(decimal salesTax, decimal importTax, int itemCount, decimal itemPrice, decimal expectedTax)
        {
            var item = new Item("Test", itemPrice, SalesTaxModifier.Imported);
            var calculator = new TaxCalculator(salesTax, importTax);

            var result = await calculator.CalculateTaxAsync(item, itemCount);
